...
- **Redesign (2026-09):** full-bleed hero (photo slideshow, word-by-word headline, floating "Find a departure" card: by trek → `/treks/:slug`, by month/length → `/treks?view=dates&month=&length=`), a marquee of promises, a statement with photo capsules, "Why small" (seat card, count-up stats), a trek mosaic from `GET /api/public/tracks` (covers still grow into the trek hero), "Meet your guide first" (guides derived from the catalog's departures at read time), how it works, FAQs, closing call. Header floats clear over the landing hero and frosts on scroll; new footer. Landing and site chrome use `--font-grotesk` (Plus Jakarta Sans). Motion lives in `index.css` and switches off under reduced motion; scroll-linked effects need `animation-timeline` support and sit still without it. No new endpoints.
- **Site-wide look and themes (2026-09):** every page uses the landing type (Plus Jakarta Sans), pill buttons and 1.5rem cards (`--card-radius`); account and admin areas use a name header over `PillTabs` instead of a sidebar. Light theme is plain white; dark theme is `<html data-theme="dark">`, chosen by the header sun/moon toggle (saved in `localStorage.theme`, else the system setting) and applied before first paint by `index.html`. Dark mode remaps the paper/stone tokens in `index.css`, so pages need no per-page dark classes.
